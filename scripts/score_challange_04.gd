...
func _ready() -> void:
	...
	$Title/title.text = translator.get_translation("challanges.challange04.title")
	$MarginContainer/VBoxContainer/Label.text = translator.get_translation("challanges.challange04.question")
...
func verificar_resposta(numero_resposta):
	if numero_resposta == resposta_correta:
		...
		database.update_rows("challanges", condition, {"challange04": "completed"})
		$ConcludedChallange.visible = true
